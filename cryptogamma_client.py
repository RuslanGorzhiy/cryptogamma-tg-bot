...
def _auth_headers() -> dict:
    """Собирает заголовки авторизации из переменных окружения.

    cryptogamma.io закрыл анонимный доступ к /api/public/snapshot —
    теперь для запросов нужен бесплатный токен, который выдаётся на
    https://cryptogamma.io/api-docs/ после входа через Google.

    Так как публичная схема авторизации не задокументирована открытым
    текстом (страница api-docs рендерится через JS и требует логина,
    чтобы показать точный формат), поддерживаются оба самых частых
    варианта — выберите тот, который реально требует сайт, посмотрев
    вкладку Network в браузере при обращении дашборда к API:

        CRYPTOGAMMA_API_TOKEN       — токен, полученный на сайте
        CRYPTOGAMMA_AUTH_SCHEME     — "bearer" (по умолчанию) или "apikey"
...
def fetch_snapshot(asset: str, timeout: int = DEFAULT_TIMEOUT) -> GammaSnapshot:
    ...
    Поднимает CryptoGammaError при сетевой ошибке, HTTP-ошибке (включая
    401 — отсутствует/неверный токен) или некорректном JSON.
    """
    ...
    try:
        ...
        if resp.status_code == 401:
            raise CryptoGammaError(
                f"401 Unauthorized для {asset}: cryptogamma.io требует API-токен. "
                "Получите бесплатный токен на https://cryptogamma.io/api-docs/ "
                "(вход через Google) и задайте его в переменной окружения "
                "CRYPTOGAMMA_API_TOKEN (см. README)."
            )
